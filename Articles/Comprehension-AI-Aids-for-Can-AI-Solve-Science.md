...
**Remark:** A fair amount of LLMs give their code results within Markdown code block delimiters (like "```".) Hence, (1) the prompt ["ThemeTableJSON"](https://www.wolframcloud.com/obj/antononcube/DeployedResources/Prompt/ThemeTableJSON/) does not use Interpreter["JSON"], but Interpreter["String"], and (2) we use above the function FromJSON in order to convert the LLM input into a WL data structure. (See the definition of FromJSON see the "Setup" section below.)
